// All valid credit card numbers
const valid1 = [4, 5, 3, 9, 6, 7, 7, 9, 0, 8, 0, 1, 6, 8, 0, 8]
const valid2 = [5, 5, 3, 5, 7, 6, 6, 7, 6, 8, 7, 5, 1, 4, 3, 9]
const valid3 = [3, 7, 1, 6, 1, 2, 0, 1, 9, 9, 8, 5, 2, 3, 6]
const valid4 = [6, 0, 1, 1, 1, 4, 4, 3, 4, 0, 6, 8, 2, 9, 0, 5]
const valid5 = [4, 5, 3, 9, 4, 0, 4, 9, 6, 7, 8, 6, 9, 6, 6, 6]

// All invalid credit card numbers
const invalid1 = [4, 5, 3, 2, 7, 7, 8, 7, 7, 1, 0, 9, 1, 7, 9, 5]
const invalid2 = [5, 7, 9, 5, 5, 9, 3, 3, 9, 2, 1, 3, 4, 6, 4, 3]
const invalid3 = [3, 7, 5, 7, 9, 6, 0, 8, 4, 4, 5, 9, 9, 1, 4]
const invalid4 = [6, 0, 1, 1, 1, 2, 7, 9, 6, 1, 7, 7, 7, 9, 3, 5]
const invalid5 = [5, 3, 8, 2, 0, 1, 9, 7, 7, 2, 8, 8, 3, 8, 5, 4]

// Can be either valid or invalid
const mystery1 = [3, 4, 4, 8, 0, 1, 9, 6, 8, 3, 0, 5, 4, 1, 4]
const mystery2 = [5, 4, 6, 6, 1, 0, 0, 8, 6, 1, 6, 2, 0, 2, 3, 9]
const mystery3 = [6, 0, 1, 1, 3, 7, 7, 0, 2, 0, 9, 6, 2, 6, 5, 6, 2, 0, 3]
const mystery4 = [4, 9, 2, 9, 8, 7, 7, 1, 6, 9, 2, 1, 7, 0, 9, 3]
const mystery5 = [4, 9, 1, 3, 5, 4, 0, 4, 6, 3, 0, 7, 2, 5, 2, 3]

// An array of all the arrays above
const batch = [valid1, valid2, valid3, valid4, valid5, invalid1, invalid2, invalid3, invalid4, invalid5, mystery1, mystery2, mystery3, mystery4, mystery5]


// Add your functions below:

const validateCred = (cardNumber) => {
  // Create a copy of the array to avoid mutation
  const copiedCardNumber = [...cardNumber];

  // Start from the rightmost digit (check digit)
  for (let i = copiedCardNumber.length - 2; i >= 0; i -= 2) {
    let doubledDigit = copiedCardNumber[i] * 2;

    // Subtract 9 if the doubled digit is greater than 9
    if (doubledDigit > 9) {
      doubledDigit -= 9;
    }

    // Update the array with the doubled and adjusted digit
    copiedCardNumber[i] = doubledDigit;
  }

  // Sum up all the digits in the array
  const sum = copiedCardNumber.reduce((acc, digit) => acc + digit, 0);

  // Check if the sum modulo 10 is 0
  return sum % 10 === 0;
};

// Example usage
const validCardNumber = [5, 2, 2, 8, 2, 8, 3, 5, 5, 6, 5, 7, 6, 6, 6, 4];
const invalidCardNumber = [4, 5, 5, 6, 7, 3, 7, 5, 8, 6, 8, 9, 9, 8, 5, 6];

console.log(validateCred(validCardNumber));   // Should return true
console.log(validateCred(invalidCardNumber)); // Should return false


const findInvalidCards = (batch) => {
  // Array to store invalid cards
  const invalidCards = [];

  // Iterate through each credit card number in the batch
  for (let cardNumber of batch) {
    // Check the validity using the previously defined validateCred function
    if (!validateCred(cardNumber)) {
      // If the card is invalid, add it to the invalidCards array
      invalidCards.push(cardNumber);
    }
  }

  // Return the array of invalid cards
  return invalidCards;
};

// Example usage
const batchOfCards = [
  [5, 2, 2, 8, 2, 8, 3, 5, 5, 6, 5, 7, 6, 6, 6, 4],
  [4, 5, 5, 6, 7, 3, 7, 5, 8, 6, 8, 9, 9, 8, 5, 6],
  // Add more card numbers as needed
];

const invalidCards = findInvalidCards(batchOfCards);
console.log(invalidCards);

const idInvalidCardCompanies = (invalidCards) => {
  // Set to store unique company names
  const uniqueCompanies = new Set();

  // Iterate through each invalid card number
  for (let cardNumber of invalidCards) {
    // Identify the company based on the first digit of the card number
    switch (cardNumber[0]) {
      case 3:
        uniqueCompanies.add('Amex');
        break;
      case 4:
        uniqueCompanies.add('Visa');
        break;
      case 5:
        uniqueCompanies.add('Mastercard');
        break;
      case 6:
        uniqueCompanies.add('Discover');
        break;
      default:
        console.log('Company not found');
    }
  }

  // Convert the set to an array and return the result
  return Array.from(uniqueCompanies);
};

// Example usage
const invalidCreditCards = [
  [5, 2, 2, 8, 2, 8, 3, 5, 5, 6, 5, 7, 6, 6, 6, 4],
  [4, 5, 5, 6, 7, 3, 7, 5, 8, 6, 8, 9, 9, 8, 5, 6],
  // Add more invalid card numbers as needed
]

const invalidCompanies = idInvalidCardCompanies(invalidCreditCards);
console.log(invalidCompanies);

















